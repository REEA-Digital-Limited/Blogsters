...
namespace Joomla\Component\Blogsters\Administrator\View\Blogster_tags;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_BLOGSTERS_TAGS_PAGE_TITLE'), 'blogster_tags');

		if ($canDo->get('core.create'))
		{
			$toolbar->addNew('blogster_tag.add');
		}
		...
		if ($canDo->get('core.edit.state'))
		{
			...
			$childBar->publish('blogster_tags.publish')->listCheck(true);
			$childBar->unpublish('blogster_tags.unpublish')->listCheck(true);
			$childBar->archive('blogster_tags.archive')->listCheck(true);

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('blogster_tags.trash')->listCheck(true);
			}
		}

		if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
		{
			$toolbar->delete('blogster_tags.delete')
