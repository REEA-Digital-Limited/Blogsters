...
namespace Joomla\Component\Blogsters\Administrator\View\Blogster_authors;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_BLOGSTERS_AUTHORS_PAGE_TITLE'), 'blogster_authors');

		if ($canDo->get('core.create'))
		{
			$toolbar->addNew('blogster_author.add');
		}
		...
		if ($canDo->get('core.edit.state'))
		{
			...
			$childBar->publish('blogster_authors.publish')->listCheck(true);
			$childBar->unpublish('blogster_authors.unpublish')->listCheck(true);
			$childBar->archive('blogster_authors.archive')->listCheck(true);

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('blogster_authors.trash')->listCheck(true);
			}
		}

		if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
		{
			$toolbar->delete('blogster_authors.delete')
